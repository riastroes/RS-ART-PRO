class App{
  ...
  App(String name){
    ...
    this.pal1.addRGBColors("zw.png", 10, 255);
  ...
  void create3(){
    ...
    for(int i = 10; i < max-58; i+=this.grid.wmax){
      ...
      this.grid.disorderByVector(this.grid.pos[i+58],6.1);
      //this.grid.disorderByVector(this.grid.pos[i],6.4);
      this.grid.disorderByVector(this.grid.pos[i+28],-1);
    }
    for(int i = this.grid.wmax*20; i < this.grid.wmax*40; i+=20){
      this.grid.disorderByVector(this.grid.pos[i+28],-3);
       this.grid.disorderByVector(this.grid.pos[i],3);
      print(i + " ");
    }
