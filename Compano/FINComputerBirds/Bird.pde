class Bird {
  ...
  Bird(PVector posleft) {
    this.posleft = posleft;
    this.posright = posOnCircle(this.posleft, 100, random(TWO_PI)); 
    this.toplegs = PVector.sub(this.posright, this.posleft).div(2);
    this.toplegs.add(this.posleft);
    this.toplegs.y -= random(200, 300);
  ...
  void drawFoot(PVector foot) {
    ...
    line(foot.x, foot.y-20, foot.x+40, foot.y + 10);
    line(foot.x, foot.y-20, foot.x-40, foot.y + 10);
    line(foot.x, foot.y-20, foot.x, foot.y + 20);
  }
  ...
  void drawLeg(PVector foot, int parts) {
    ...
    if(parts == 0){
      line(foot.x, foot.y, this.toplegs.x, this.toplegs.y);
        ...
        float up = random(100,180);
        knee.y -= up;
        knee.x += random(-230,230);
        if(up > 140){
        foot.y -= 100;
        }
  ...
  void drawBody() {
    noStroke();
    fill(255, 0, 0);
    this.center = new PVector(this.toplegs.x, this.toplegs.y - 60);
