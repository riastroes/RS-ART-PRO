...
class Grid{
   ...
   Grid(PVector pos, float maxwidth, float maxheight, int wmax, int hmax){
    ...
    for(int h = 0; h < this.hmax; h++){
      for( int w = 0 ; w < this.wmax; w++){
        x = ( w * this.cellwidth);
        y = ( h * this.cellheight) + (w * (this.cellheight/this.wmax));
  ...
  void disorderHeight(int row, float factor){
    ...
    for(int i = 0; i < p.length; i++){
      p[i] = new PVector(0,0);
      p[i].x = this.center.x + (((this.cylinderwidth/2)) * sin(anglestep * i));
      p[i].y = this.center.y + (((this.cylinderheight/2)) * cos(anglestep * i));
    }
    
    
    for( int w = 0 ; w < this.wmax; w++){
      this.pos[(row* this.wmax)+ w].x =  p[w].x;
      this.pos[(row* this.wmax)+ w].y = p[w].y + (row*this.cellheight) +factor;
    }
