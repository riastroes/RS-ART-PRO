class GenKnitting{
  ...
  GenKnitting(PVector first, int maxrows){
    ...
    this.knitting.add(new PVector(0,0,0));
 ...
 void createSkirt(int rows, int stitches, int marge){
      ...
      if(int(this.knitting.get(k).z) == 0){
        ...
        if(abs(v.mag()) > 0){
          ...
          if(this.knitting.get(k).z == 1){
             gcode.extrude += v.mag() * layerheight * (thickness+0.3) ;
          }
          else if(this.knitting.get(k).z == 2){
  ...
  void animateKnitting(){
    ...
    for(int i = 1; i <  this.knitting.size(); i++){
        ...
        else if(int(this.knitting.get(i).z) == 2){
           strokeWeight(1);
           stroke(0);
        }
