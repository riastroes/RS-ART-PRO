...
class Grid{
  ...
  void projectOnHalfCircle(PVector center, float radius){
    for(int h =0; h< this.hmax; h++){
      for(int w =0; w< this.wmax; w++){
      this.pos[(h* this.wmax)+ w].x = center.x +((radius - (radius/this.hmax) * h) *sin((PI/this.wmax)*w));
      this.pos[(h* this.wmax)+ w].y = center.y +((radius - (radius/this.hmax) * h) * cos((PI/this.wmax)*w));
      }
    }
  }
