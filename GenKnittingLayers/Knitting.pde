class Knitting{
  ...
  Knitting(PVector pos, float pwidth, float pheight, int maxrows, int maxstitches){
   ...
   // this.first = new PVector(pos.x + 5, pos.y + 5 + (this.kheight/(this.maxrows/2)));
    ...
    for(int i = 0; i < this.rows.length; i++){
      sign = -1 * sign;
      this.rows[i] = new Row(i, this.next, this.kwidth, this.kheight/(this.maxrows/2), this.maxstitches, sign);
      if(i == 0){
